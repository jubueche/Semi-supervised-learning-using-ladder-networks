...
layer_sizes = [128, 1200, 600, 300, 300, 300, 10]
...
denoising_cost = [1000.0, 10.0, 1, 0.10, 0.10, 0.10, 0.10]
...
for i in tqdm(range(i_iter, num_iter)):
    ...
    if (i > 1) and ((i+1) % (num_iter/num_epochs) == 0):
        print "Accuracy after ", num_epochs, ": ", sess.run(accuracy, feed_dict={inputs: mnist.test.images, outputs: mnist.test.labels, training: False}), "%"
...
c = sess.run(predict, feed_dict={inputs: mnist.validation.images, outputs: mnist.validation.labels, training: False})
df = pd.DataFrame(c)
df.index += 30000
df.to_csv("prediction.csv", ",", header=["y"], index_label="Id")
sess.close()
